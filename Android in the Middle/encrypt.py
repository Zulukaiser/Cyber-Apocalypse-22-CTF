from Crypto.Cipher import AES
from Crypto.Util.number import long_to_bytes
import hashlib
import random
import socketserver
import signal

FLAG = "HTB{--REDACTED--}"
DEBUG_MSG = "DEBUG MSG - "
p = 
g = 2
text = "Initialization Sequence - Code 0"

def decrypt(encrypted, shared_secret):
    key = hashlib.md5(long_to_bytes(shared_secret)).digest()
    cipher = AES.new(key, AES.MODE_ECB)
    message = cipher.decrypt(encrypted)
    return message

def encrypt(plain, shared_secret):
	key = hashlib.md5(long_to_bytes(shared_secret)).digest()
	cipher = AES.new(key, AES.MODE_ECB)
	encrypted_message = cipher.encrypt(plain.encode(encoding = 'UTF-8'))
	return encrypted_message

print(f"{DEBUG_MSG} Generating The Global DH Parameters\n")
print(f"{DEBUG_MSG} g = {g}, p = {p}\n")
print(f"{DEBUG_MSG} Calculation Complete\n\n")

print(f"{DEBUG_MSG} Generating The Public Key of CPU...\n")
c = random.randrange(2, p - 1)
C = pow(g, c, p)
print(f'{DEBUG_MSG} Calculation Complete\n')
print(f'{DEBUG_MSG} Public Key is: ???\n\n')

M = input("Enter The Public Key of The Memory: ")

try:
    M = int(M)
except:
    print(f'{DEBUG_MSG} Unexpected Error Occured\n')
    exit()
print(f"\n{DEBUG_MSG} The CPU Calculates The Shared Secret\n")
shared_secret = pow(M, c, p)
print(f"{DEBUG_MSG} Calculation Complete\n\n")
#encrypted_sequence = recieveMessage(
#    s, "Enter The Encrypted Initialization Sequence: ")

#try:
#    encrypted_sequence = bytes.fromhex(encrypted_sequence)
#    assert len(encrypted_sequence) % 16 == 0
#except:
#    print(s, DEBUG_MSG + "Unexpected Error Occured\n")
#    exit()
sequence = encrypt(text, shared_secret)
print(f'{sequence.hex()}\n')
decrypted = decrypt(sequence, shared_secret)
print(f'{decrypted}')
if decrypted == b"Initialization Sequence - Code 0":
    print(f"{DEBUG_MSG} Reseting The Protocol With The New Shared Key\n")
    print(f"{DEBUG_MSG} {FLAG}")
else:
    exit()